...
echo "[process 1/17] Processing country: $country_files"

echo "[process 2/17] Downloading countryInfo.txt for $country_files..."
conditional_download "https://download.geonames.org/export/dump/countryInfo.txt" "$DOWNLOAD_DIR/countryInfo.txt"

echo "[process 3/17] Validating country code: $country_files"
...
echo "[process 4/17] Cleaning up previous data files for $country_files..."
...
for cfile in $country_files; do
    echo "[process 5/17] Processing country file: $cfile"
...
echo "[process 6/17] Creating foreign keys for administrative divisions for $country_files..."
...
echo "[process 7/17] Extracting geonameIDs for alternate names filtering for $country_files..."
awk -F'\t' 'NR>1 {print $1}' "$DATA_DIR/geonamesplus_${country_files}.csv" > "$DATA_DIR/geonameids_${country_files}.txt"

echo "[process 8/17] Downloading and filtering alternate names data for $country_files..."
...
echo "[process 9/17] Splitting alternate names into chunks for $country_files..."
...
echo "[process 10/17] Downloading and processing admin1 codes..."
...
echo "[process 11/17] Downloading and processing admin2 codes..."
...
echo "[process 12/17] Generating admin3 and admin4 codes for $country_files..."
...
echo "[process 13/17] Adding headers to admin3/admin4 files and extracting country information for $country_files..."
...
echo "[process 14/17] Downloading and processing hierarchy data for $country_files..."
...
echo "[process 15/17] Computing best English alternate names for $country_files..."
source ./compute-bestnames.sh "$country_files"


echo "[process 16/17] Splitting data with best names into chunks of $CHUNK_SIZE records each for $country_files..."
...
echo "[process 17/17] Adding headers to each chunk for $country_files..."
